...
constexpr int kGPUThreadSize = 512;
constexpr int kBoxBlockSize = 7;

inline void CenterPointTrtCudaCheck(cudaError_t err, const char* file,
                                    int line) {
  CHECK_EQ(err, cudaSuccess) << "CUDA error: " << cudaGetErrorString(err)
                             << " at " << file << ":" << line;
}

#define CENTERPOINT_TRT_CUDA_CHECK(call)                                 \
  ::apollo::perception::lidar::CenterPointTrtCudaCheck((call), __FILE__, \
                                                       __LINE__)
...
static __device__ __forceinline__ float cp_trt_atomicAdd(float* address,
                                                         float val) {
...
static __device__ __forceinline__ int cp_trt_atomicAdd(int* address, int val) {
...
static __device__ __forceinline__ double cp_trt_atomic_exch(double* addr,
                                                            double val) {
...
static __device__ __forceinline__ float cp_trt_atomic_exch(float* addr,
                                                           float val) {
  return atomicExch(addr, (val));
}

static __device__ __forceinline__ void cp_trt_atomicMax(float* max_height_addr,
                                                        float pz) {
  float old_pz = *max_height_addr;
  do {
    old_pz = cp_trt_atomic_exch(max_height_addr, (pz));
    if (pz < old_pz) {
      pz = old_pz;
    }
...
static __host__ __device__ __forceinline__ float cp_trt_sigmoid(float x) {
  return 1.0f / (1.0f + expf(-x));
}

static __host__ __device__ __forceinline__ float cp_trt_square_root(float x,
                                                                    float y) {
  return sqrtf(x * x + y * y);
}

static __host__ __device__ __forceinline__ float cp_trt_jaccard_overlap(
    const float* bbox1, const float* bbox2) {
...
// rotate_overlap
__host__ __device__ float trangle_area(float* a, float* b, float* c) {
  return ((a[0] - c[0]) * (b[1] - c[1]) - (a[1] - c[1]) * (b[0] - c[0])) / 2.f;
}

__host__ __device__ void sort_vertex_in_convex_polygon(float* int_pts,
                                                       int num_of_inter) {
  if (num_of_inter == 0) {
    return;
  }
...
__host__ __device__ bool line_segment_intersection(const float* pts1,
                                                   const float* pts2, int i,
                                                   int j, float* temp_pts) {
  float A0 = pts1[2 * i];
  float A1 = pts1[2 * i + 1];
...
__host__ __device__ bool point_in_quadrilateral(float pt_x, float pt_y,
                                                const float* corners) {
...
__host__ __device__ float rotate_inter_gpu(const float* corners1,
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      if (line_segment_intersection(corners1, corners2, i, j,
...
__global__ void MapKernel(const int n, const base::PointF* pc_gpu,
                          const int* point2grid, Dtype* max_height_data,
                          Dtype* mean_height_data, Dtype* mean_intensity_data,
                          Dtype* count_data) {
  CUDA_KERNEL_LOOP(i, n) {
    ...
    cp_trt_atomicMax(&max_height_data[idx], pz);
    cp_trt_atomicAdd(&mean_height_data[idx], pz);
    if (mean_intensity_data != nullptr) {
      cp_trt_atomicAdd(&mean_intensity_data[idx], pi);
    }
    cp_trt_atomicAdd(&count_data[idx], (Dtype)1);
  }
}
...
__global__ void TopIntensityKernel(const int n, const base::PointF* pc_gpu,
                                   const int* point2grid,
                                   Dtype* top_intensity_data,
                                   Dtype* max_height_data) {
  CUDA_KERNEL_LOOP(i, n) {
    int idx = point2grid[i];
    if (idx == -1) {
      continue;
...
__global__ void HeightBinKernel(const int n, const int map_size,
                                const float height_bin_min_height,
                                const float height_bin_voxel_size,
                                const int height_bin_dim,
  ...
  CUDA_KERNEL_LOOP(i, n) {
    ...
    Dtype pz = pc_gpu[i].z;
    int height_bin_index =
        static_cast<int>((pz - height_bin_min_height) / height_bin_voxel_size);
    height_bin_index = height_bin_index < 0 ? 0 : height_bin_index;
    height_bin_index = height_bin_index >= height_bin_dim ? (height_bin_dim - 1)
                                                          : height_bin_index;
    height_bin_data[height_bin_index * map_size + idx] = 1.f;
  }
}

__global__ void Point2GridKernel(
    const base::PointF* pc_gpu, const int cloud_size, const float x_min_range,
    const float y_min_range, const float z_min_range, const float z_max_range,
    const float voxel_x_size, const float voxel_y_size, const int grid_x_size,
    const int grid_y_size, const bool enable_rotate_45degree, int* point2grid) {
  CUDA_KERNEL_LOOP(i, cloud_size) {
    ...
    if (pos_yf < 0 || pos_y >= grid_y_size || pos_xf < 0 ||
        pos_x >= grid_x_size) {
      continue;
    }
...
__global__ void PointCloudSumKernel(const base::PointF* pc_gpu,
                                    ...
                                    const bool enable_rotate_45degree,
                                    const int* point2grid,
                                    int* grid2pointnum_data,
                                    Dtype* voxels_cluster_data) {
  CUDA_KERNEL_LOOP(i, cloud_size) {
    int idx = point2grid[i];
    if (idx == -1) {
      continue;
    }
    cp_trt_atomicAdd(&grid2pointnum_data[idx], 1);
    ...
    cp_trt_atomicAdd(&voxels_cluster_data[map_size * 0 + idx], px);
    cp_trt_atomicAdd(&voxels_cluster_data[map_size * 1 + idx], py);
    cp_trt_atomicAdd(&voxels_cluster_data[map_size * 2 + idx], pc_gpu[i].z);
  }
}

template <typename Dtype>
__global__ void VoxelFeatureKernel(
    const int cloud_size, const int map_size, const int grid_x_size,
    const float voxel_x_size, const float voxel_y_size, const float x_offset,
    const float y_offset, const float max_x_range, const float max_y_range,
    const float max_z_range, const int voxel_feature_dim,
    const bool use_input_norm, const bool enable_rotate_45degree,
    const base::PointF* pc_gpu, const int* point2grid,
    const int* grid2pointnum_data, Dtype* voxels_cluster_data,
    Dtype* voxels_data) {
  CUDA_KERNEL_LOOP(i, cloud_size) {
    ...
    float point_num =
        max(1.f, static_cast<float>(grid2pointnum_data[grid_idx]));
    ...
    voxels_data[i * voxel_feature_dim + 7] =
        Dtype(px) - (static_cast<Dtype>(coord_x) * voxel_x_size + x_offset);
    voxels_data[i * voxel_feature_dim + 8] =
        Dtype(py) - (static_cast<Dtype>(coord_y) * voxel_y_size + y_offset);
    if (use_input_norm) {
...
__global__ void CanvasFeatureKernel(
    const int n, const int pillar_feature_dim, const int map_size,
  ...
  CUDA_KERNEL_LOOP(i, n) {
    ...
    for (int c_idx = 0; c_idx < pillar_feature_dim; c_idx++) {
      ...
      cp_trt_atomicMax(&canvas_feature_data[canvas_idx],
                       pillar_feature_data[pillar_idx]);
    }
  }
}

__global__ void InitCanvasFeatureKernel(
    const int n, const int cloud_size, const int voxel_feature_dim,
    const int map_size, const int grid_x_size, const int max_points_in_voxel,
    const int* grid2pointnum_data, const float* pillar_feature_data,
    float* canvas_feature_data) {
  CUDA_KERNEL_LOOP(i, n) {
...
// rotate overlap
__global__ void ComputeRotateOverlapKernel(
    const int nthreads, const int box_block_size, const int box_num,
  ...
  CUDA_KERNEL_LOOP(idx, nthreads) {
    ...
    float area_inter =
        rotate_inter_gpu(box_corners + start_loc_i, box_corners + start_loc_j);
...
__global__ void TransBoxforNmsKernel(const int box_num, const float quantize,
                                     const float* res_box_data,
                                     float* box_corner, float* box_for_nms,
                                     float length_enlarge,
                                     float width_enlarge) {
...
__global__ void ComputeOverlapKernel(const int nthreads, const int box_num,
  ...
  CUDA_KERNEL_LOOP(idx, nthreads) {
    ...
    float overlap = cp_trt_jaccard_overlap(box_for_nms + start_loc_i,
                                           box_for_nms + start_loc_j);
    overlapped_data[idx] = overlap;
  }
}

__global__ void Points2BoxidKernel(
    const int cloud_size, const bool enable_rotate_45degree, const int box_num,
  ...
  CUDA_KERNEL_LOOP(point_idx, cloud_size) {
    ...
    for (int i = 0; i < box_num; i++) {
      ...
      if ((angle1 < 0 && angle2 < 0 && angle3 < 0 && angle4 < 0) ||
        ...
        // cp_trt_atomicAdd(valid_point_num_data, 1);
        // point2boxid_data[point_idx] = i;
        int count = cp_trt_atomicAdd(valid_point_num_data, 1);
...
__global__ void SimplePoints2BoxKernel(
    const int cloud_size, const bool enable_rotate_45degree, const int box_num,
    const base::PointF* pc, const int* kept_indices, const float* res_box_data,
    const float* box_corner, const float* box_for_nms,
    int* valid_point_num_data, int* valid_point2boxid_data,
...
__global__ void GetAllRemainBoxInfoKernel(
    const int nthreads, const int all_res_num, const float* res_box_data,
    const float* res_conf_data, const int* res_cls_data,
    float* all_res_box_data, float* all_res_conf_data, int* all_res_cls_data) {
  CUDA_KERNEL_LOOP(idx, nthreads) {
...
__global__ void DecodeObjectKernel(
    const int map_size, const int nms_pre_max_size, const float min_x_range,
    const float min_y_range, const float voxel_x_size, const float voxel_y_size,
    const int head_x_size, const int head_y_size, const int downsample_size,
    const int num_classes_in_task, const int cls_range,
    const float* score_thresh, const float* reg, const float* hei,
    const float* dim, const float* cls, const float* rot, int* res_box_num_data,
    float* res_box_data, float* res_conf_data, int* res_cls_data) {
  CUDA_KERNEL_LOOP(idx, map_size) {
    ...
    float conf = cp_trt_sigmoid(max_score);
    if (conf > score_thresh[label]) {
      int cur_valid_box_id = cp_trt_atomicAdd(res_box_num_data, 1);
      // printf("curr conf: %.2f, label: %d, box_num: %d\n", conf, label,
      // res_box_num_data[0]); Fixme:
      //    if the quantity of valid box is too large, there is a risk of box
      //    loss
      if (cur_valid_box_id < nms_pre_max_size) {
        res_box_data[cur_valid_box_id * kBoxBlockSize + 0] =
            (reg[idx + 0 * map_size] + coor_x) * downsample_size *
                voxel_x_size +
            min_x_range;
        res_box_data[cur_valid_box_id * kBoxBlockSize + 1] =
            (reg[idx + 1 * map_size] + coor_y) * downsample_size *
                voxel_y_size +
            min_y_range;
        res_box_data[cur_valid_box_id * kBoxBlockSize + 2] = hei[idx];
        res_box_data[cur_valid_box_id * kBoxBlockSize + 3] =
            expf(dim[idx + 0 * map_size]);
        res_box_data[cur_valid_box_id * kBoxBlockSize + 4] =
            expf(dim[idx + 1 * map_size]);
        res_box_data[cur_valid_box_id * kBoxBlockSize + 5] =
            expf(dim[idx + 2 * map_size]);
        res_box_data[cur_valid_box_id * kBoxBlockSize + 6] =
            atan2f(rot[idx], rot[idx + map_size]);
...
void CenterPointTrtCudaContext::GeneratePfnFeatureGPU() {
  float* canvas_feature_data = canvas_feature_blob_->mutable_gpu_data();
  float* voxels_cluster_data =
      canvas_feature_data + feature_offset_["canvas_feature"];
  int* grid2pointnum_data = grid2pointnum_blob_->mutable_gpu_data();

  CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
      voxels_cluster_data, 0.f, sizeof(float) * 3 * grid_x_size_ * grid_y_size_,
      stream_));
  CENTERPOINT_TRT_CUDA_CHECK(
      cudaMemsetAsync(grid2pointnum_data, 0,
                      sizeof(int) * grid_x_size_ * grid_y_size_, stream_));

  int cloud_size = cur_cloud_ptr_->size();
  // pc_gpu
  if (pc_gpu_ != nullptr) {
    CENTERPOINT_TRT_CUDA_CHECK(cudaFree(pc_gpu_));  // free cuda memory
  }
  // memory malloc and copy from cur_cloud_ptr_ to pc_gpu_
  CENTERPOINT_TRT_CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&pc_gpu_),
                                        cloud_size * sizeof(base::PointF)));
  CENTERPOINT_TRT_CUDA_CHECK(cudaMemcpy(pc_gpu_, &(cur_cloud_ptr_->front()),
  ...
  CENTERPOINT_TRT_CUDA_CHECK(
      cudaMemsetAsync(point2grid_data, 0, sizeof(int) * cloud_size, stream_));

  voxels_blob_->Reshape(std::vector<int>{cloud_size, 1, voxel_feature_dim_, 1});
  float* voxel_gpu = voxels_blob_->mutable_gpu_data();
  CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
      voxel_gpu, 0.f, sizeof(float) * cloud_size * voxel_feature_dim_,
      stream_));

  // x,y,z means. shape: (4, pillar_num), 4: [x, y, z, point_number]
  // float* xyzs = nullptr;
  // CENTERPOINT_TRT_CUDA_CHECK(cudaMalloc((void**)&xyzs, sizeof(float) *
  // grid_x_size_ * grid_y_size_ * 4));
  // CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(xyzs, 0.0f, sizeof(float) *
  // grid_x_size_ * grid_y_size_ * 4, stream_));

  int block_size = DIVUP(cloud_size, kGPUThreadSize);
  Point2GridKernel<<<block_size, kGPUThreadSize, 0, stream_>>>(
      pc_gpu_, cloud_size, x_min_range_, y_min_range_, z_min_range_,
      z_max_range_, voxel_x_size_, voxel_y_size_, grid_x_size_, grid_y_size_,
  ...
  if (use_cnnseg_features_) {
    float* max_height_data =
        canvas_feature_data + feature_offset_["max_height"];
    float* mean_height_data =
        canvas_feature_data + feature_offset_["mean_height"];
    float* top_intensity_data =
        canvas_feature_data + feature_offset_["top_intensity"];
    float* mean_intensity_data =
        canvas_feature_data + feature_offset_["mean_intensity"];
    float* count_data = canvas_feature_data + feature_offset_["count"];
    float* nonempty_data = canvas_feature_data + feature_offset_["nonempty"];
    float* height_bin_data =
        canvas_feature_data + feature_offset_["height_bin"];

    CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
        max_height_data, -5.f, sizeof(float) * map_size_, stream_));
    CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
        mean_height_data, 0.f, sizeof(float) * map_size_, stream_));
    CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
        top_intensity_data, 0.f, sizeof(float) * map_size_, stream_));
    CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
        mean_intensity_data, 0.f, sizeof(float) * map_size_, stream_));
    CENTERPOINT_TRT_CUDA_CHECK(
        cudaMemsetAsync(count_data, 0.f, sizeof(float) * map_size_, stream_));
    CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
        nonempty_data, 0.f, sizeof(float) * map_size_, stream_));
    CENTERPOINT_TRT_CUDA_CHECK(
        cudaMemsetAsync(height_bin_data, 0.f,
    ...
    TopIntensityKernel<float><<<block_size, kGPUThreadSize, 0, stream_>>>(
        cloud_size, pc_gpu_, point2grid_blob_->gpu_data(), top_intensity_data,
        max_height_data);
    cudaDeviceSynchronize();
    ...
    HeightBinKernel<float><<<block_size, kGPUThreadSize, 0, stream_>>>(
        cloud_size, map_size_, height_bin_min_height_, height_bin_voxel_size_,
        height_bin_dim_, pc_gpu_, point2grid_blob_->gpu_data(),
        height_bin_data);
    cudaDeviceSynchronize();
  }

  PointCloudSumKernel<float><<<block_size, kGPUThreadSize, 0, stream_>>>(
      pc_gpu_, cloud_size, map_size_, enable_rotate_45degree_,
  ...
  // DEVICE_SAVE<float>(voxel_gpu, cloud_size, voxel_feature_dim_,
  // std::to_string(lidar_frame_ref_->timestamp) + "_voxel_feature.txt");

  // output
  pfn_pillar_feature_blob_->Reshape(
      {static_cast<int>(cloud_size), pillar_feature_dim_});
  // float* pfn_data = pfn_pillar_feature_blob_->mutable_gpu_data();
  // CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(pfn_data, 0.f, sizeof(float) *
  // cloud_size * pillar_feature_dim_, stream_));
  // DEVICE_SAVE<float>(pfn_pillar_feature_blob_->gpu_data(), cloud_size,
  // pillar_feature_dim_, std::to_string(lidar_frame_ref_->timestamp) +
  // "_pfn1_feature.txt");
}

void CenterPointTrtCudaContext::GenerateBackboneFeatureGPU(
  ...
  // DEVICE_SAVE<float>(pillar_feature_data, cloud_size_, pillar_feature_dim_,
  // std::to_string(lidar_frame_ref_->timestamp) + "_pfn_feature.txt");
  // DEVICE_SAVE<int>(point2grid_blob_->gpu_data(), cloud_size_, 1,
  // std::to_string(lidar_frame_ref_->timestamp) + "_p2g_feature.txt");
  ...
  CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
      canvas_feature_data, 0.f, sizeof(float) * map_size_ * pillar_feature_dim_,
      stream_));

  int cloud_size = cur_cloud_ptr_->size();
  int block_size = DIVUP(cloud_size, kGPUThreadSize);
  CanvasFeatureKernel<<<block_size, kGPUThreadSize, 0, stream_>>>(
      cloud_size, pillar_feature_dim_, map_size_, grid_x_size_, point2grid_data,
      pillar_feature_data, canvas_feature_data);
  cudaDeviceSynchronize();
  ...
  // DEVICE_SAVE<float>(canvas_feature_data, shape, map_size_,
  // std::to_string(lidar_frame_ref_->timestamp) + "_canvas1_feature.txt",
  // true); DEVICE_SAVE<float>(canvas_feature_blob_->gpu_data(), shape,
  // map_size_, std::to_string(lidar_frame_ref_->timestamp) +
  // "_canvas_feature.txt", true); output
...
void CenterPointTrtCudaContext::AssignPoints2Boxid(
  ...
  TransBoxforNmsKernel<<<block_size, kGPUThreadSize, 0, stream_>>>(
      all_res_box_num, 0.2f, all_res_box_blob_->gpu_data(), box_corner,
      box_for_nms, length_enlarge_value_, width_enlarge_value_);
  ...
  CENTERPOINT_TRT_CUDA_CHECK(
      cudaMemsetAsync(valid_point_num_data, 0, sizeof(int), stream_));
  CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
      valid_point_indices_data, -1, sizeof(int) * total_cloud_size_, stream_));
  CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
      valid_point2boxid_data, -1, sizeof(int) * total_cloud_size_, stream_));
  CENTERPOINT_TRT_CUDA_CHECK(
      cudaMemcpyAsync(kept_indices_data, &kept_indices[0],
                      sizeof(int) * box_num, cudaMemcpyHostToDevice, stream_));

  base::PointF* pc_gpu = nullptr;
  if (pc_gpu != nullptr) {
    CENTERPOINT_TRT_CUDA_CHECK(cudaFree(pc_gpu));  // free cuda memory
  }
  // memory malloc and copy from original_cloud_ to pc_gpu
  CENTERPOINT_TRT_CUDA_CHECK(
      cudaMalloc(reinterpret_cast<void**>(&pc_gpu),
                 total_cloud_size_ * sizeof(base::PointF)));
  CENTERPOINT_TRT_CUDA_CHECK(cudaMemcpy(
      pc_gpu, &(original_cloud_->front()),
      sizeof(base::PointF) * total_cloud_size_, cudaMemcpyHostToDevice));
  block_size = DIVUP(total_cloud_size_, kGPUThreadSize);
  Points2BoxidKernel<<<block_size, kGPUThreadSize, 0, stream_>>>(
      total_cloud_size_, enable_rotate_45degree_, box_num, pc_gpu,
      kept_indices_data, all_res_box_blob_->gpu_data(),
      box_corner_blob_->gpu_data(), box_for_nms_blob_->gpu_data(),
      valid_point_num_data, valid_point2boxid_data, valid_point_indices_data,
      bottom_enlarge_value_, top_enlarge_value_);

  CENTERPOINT_TRT_CUDA_CHECK(cudaFree(pc_gpu));  // free cuda memory
}

void CenterPointTrtCudaContext::SimpleAssignPoints2Boxid(
    const std::vector<int>& kept_indices) {
  // int all_res_box_num = all_res_box_blob_->shape(0);
  // box_corner_blob_->Reshape(std::vector<int>{all_res_box_num, 8});
  // box_for_nms_blob_->Reshape(std::vector<int>{all_res_box_num, 4});
  ...
  SimpleTransBoxKernel<<<block_size, kGPUThreadSize, 0, stream_>>>(
      box_num, 0.2f, kept_indices_data, all_res_box_blob_->gpu_data(),
      box_corners, box_rects, length_enlarge_value_, width_enlarge_value_);
  ...
  CENTERPOINT_TRT_CUDA_CHECK(
      cudaMemsetAsync(valid_point_num_data, 0, sizeof(int), stream_));
  CENTERPOINT_TRT_CUDA_CHECK(
      cudaMemsetAsync(valid_point_indices_data, -1,
                      sizeof(int) * max_valid_point_size_, stream_));
  CENTERPOINT_TRT_CUDA_CHECK(
      cudaMemsetAsync(valid_point2boxid_data, -1,
                      sizeof(int) * max_valid_point_size_, stream_));
  ...
  block_size = DIVUP(total_cloud_size_, kGPUThreadSize);
  SimplePoints2BoxKernel<<<block_size, kGPUThreadSize, 0, stream_>>>(
      total_cloud_size_, enable_rotate_45degree_, box_num, pc_gpu,
      kept_indices_data, all_res_box_blob_->gpu_data(),
      box_corners_blob_->gpu_data(), box_rects_blob_->gpu_data(),
      valid_point_num_data, valid_point2boxid_data, valid_point_indices_data,
      bottom_enlarge_value_, top_enlarge_value_);
...
void CenterPointTrtCudaContext::ApplyRotateNms(
    const bool* rotate_overlapped, const int valid_box_num,
    const int* all_sorted_indices, std::vector<int>* box_reverve_flag,
...
int CenterPointTrtCudaContext::ApplyNmsGPU(const int box_num_pre,
                                           const int all_res_num,
                                           std::vector<int>* kept_indices) {
  // switch current gpu device data
  // int gpu_id = base::GetDevice();
  // auto par =
  // thrust::cuda::par(GPUAllocator::Instance().GetAllocator(gpu_id));
  if (box_num_pre <= 0) {
    return 0;
  }
  ...
  TransBoxforNmsKernel<<<block_size, kGPUThreadSize, 0, stream_>>>(
      box_num_pre, -1.f, res_box_blob_->gpu_data(), box_corner, box_for_nms,
      -1.f, -1.f);
  ...
  CENTERPOINT_TRT_CUDA_CHECK(cudaMemcpyAsync(
      remain_conf_data, res_conf_blob_->gpu_data(), sizeof(float) * box_num_pre,
  ...
  thrust::sequence(thrust::device, res_sorted_indices_data,
                   res_sorted_indices_data + box_num_pre);
  thrust::sort_by_key(thrust::device, remain_conf_data,
                      remain_conf_data + box_num_pre, res_sorted_indices_data,
  ...
  ComputeRotateOverlapKernel<<<block_size, kGPUThreadSize, 0, stream_>>>(
      nthreads, kBoxBlockSize, box_num_pre, nms_overlap_thresh_,
      res_box_blob_->gpu_data(), box_corner_blob_->gpu_data(),
      res_sorted_indices_blob_->gpu_data(),
  ...
  ApplyRotateNms(rotate_overlapped_blob_->cpu_data(), box_num_pre,
                 res_sorted_indices, &box_reserve_flag, true);
...
void CenterPointTrtCudaContext::DecodeValidObjects(
    std::vector<int>* kept_indices) {
  ...
  // DEVICE_SAVE<float>(out_box_blob->gpu_data(), head_x_size_ * head_y_size_,
  // 18, "05_bbox_preds.txt"); DEVICE_SAVE<float>(out_cls_blob->gpu_data(),
  // head_x_size_ * head_y_size_, 4, "06_cls_scores.txt");
  // DEVICE_SAVE<float>(out_dir_blob->gpu_data(), head_x_size_ * head_y_size_,
  // 6, "07_dir_scores.txt");
  ...
  CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
      all_res_box_data, 0.f, sizeof(float) * max_candidate_num_ * kBoxBlockSize,
      stream_));
  CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
      all_res_conf_data, 0.f, sizeof(float) * max_candidate_num_, stream_));
  CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
      all_res_cls_data, -1, sizeof(int) * max_candidate_num_, stream_));

  kept_indices->reserve(num_tasks_ * nms_post_max_size_);
  int all_res_box_num = 0;
  for (int i = 0; i < num_tasks_; ++i) {
    const float* reg = output_box_blob_->gpu_data() +
                       output_box_blob_->offset(0, box_range * i);
    const float* hei =
        output_box_blob_->gpu_data() +
        output_box_blob_->offset(0, box_range * i + head_map_["reg"]);
    const float* dim =
        output_box_blob_->gpu_data() +
        output_box_blob_->offset(
            0, box_range * i + head_map_["reg"] + head_map_["hei"]);
    const float* cls = output_cls_blob_->gpu_data() +
                       output_cls_blob_->offset(0, cls_range[i]);
    const float* rot = output_dir_blob_->gpu_data() +
                       output_dir_blob_->offset(0, dir_range * i);
    ...
    float* score_thresh_per_class_data =
        score_class_map_blob_->mutable_gpu_data();

    CENTERPOINT_TRT_CUDA_CHECK(
        cudaMemsetAsync(res_box_num_data, 0, sizeof(int), stream_));
    CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
        res_box_data, 0.f, sizeof(float) * nms_pre_max_size_ * kBoxBlockSize,
        stream_));
    CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
        res_conf_data, 0.f, sizeof(float) * nms_pre_max_size_, stream_));
    CENTERPOINT_TRT_CUDA_CHECK(cudaMemsetAsync(
        res_cls_data, -1, sizeof(int) * nms_pre_max_size_, stream_));
    CENTERPOINT_TRT_CUDA_CHECK(cudaMemcpyAsync(
        score_thresh_per_class_data, &score_thresh_map_[0],
        sizeof(float) * num_classes_, cudaMemcpyHostToDevice, stream_));

    int block_size = DIVUP(head_map_size_, kGPUThreadSize);
    DecodeObjectKernel<<<block_size, kGPUThreadSize, 0, stream_>>>(
        head_map_size_, nms_pre_max_size_, x_min_range_, y_min_range_,
        voxel_x_size_, voxel_y_size_, head_x_size_, head_y_size_,
        downsample_size_, num_classes_in_task_[i], cls_range[i],
        score_thresh_per_class_data, reg, hei, dim, cls, rot, res_box_num_data,
        res_box_data, res_conf_data, res_cls_data);

    CENTERPOINT_TRT_CUDA_CHECK(cudaStreamSynchronize(stream_));
    const int* res_box_num = res_box_num_blob_->cpu_data();
    // const int* res_box_num = res_box_num_blob_->gpu_data();
    int box_num_pre =
        res_box_num[0] > nms_pre_max_size_ ? nms_pre_max_size_ : res_box_num[0];
    if (box_num_pre <= 0) {
      continue;
    }
    ...
    GetAllRemainBoxInfoKernel<<<block_size, kGPUThreadSize, 0, stream_>>>(
        box_num_pre, all_res_box_num, res_box_data, res_conf_data, res_cls_data,
